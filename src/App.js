...
function App() {
  ...
  useEffect(() => {
    const fetchPrediction = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1:8000/api/predict/?spend=${marketingBudget}&coins=${Cashback}`);
        const pred = response.data.predicted_bookings;
        setPredictedBookings(pred);
        generateGraphData(marketingBudget, pred);
      } catch (error) {
        // Fallback for Demo
        const simulatedPred = 100 + (parseInt(marketingBudget) / 50) + (parseInt(Cashback) / 10);
  ...
  }, [marketingBudget, Cashback, activeTab]);
  ...
  const runOptimizer = () => {
    ...
    const currentCoins = parseInt(Cashback);
          ...
          <div className="home-container">
            ...
            <div className="features-row">
              <div className="feat-card">
                ...
                <p>Deep learning models that analyze regional demand patterns to forecast bookings.</p>
              </div>
              <div className="feat-card">
